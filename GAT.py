...
# 7. UltraEnhancedGAT model
class UltraEnhancedGAT(nn.Module):
    def __init__(self, node_in, graph_in, hidden=256, heads=4, dropout=0.03):
        super().__init__()
        self.gat1 = GATConv(node_in, hidden, heads=heads, concat=True)
        self.bn1 = nn.BatchNorm1d(hidden * heads)
        self.gat2 = GATConv(hidden * heads, hidden, heads=heads, concat=True)
        self.bn2 = nn.BatchNorm1d(hidden * heads)
        self.gat3 = GATConv(hidden * heads, hidden, heads=heads, concat=True)
        self.bn3 = nn.BatchNorm1d(hidden * heads)
        self.gat4 = GATConv(hidden * heads, hidden, heads=1, concat=False)
    ...
    def forward(self, data):
        ...
        # Final GAT: output is [N, hidden], x3 is [N, hidden*heads]
        x3_proj = self.skip_proj(x3)
        x4 = F.elu(self.bn4(self.gat4(x3, edge_index))) + x3_proj
...
model = UltraEnhancedGAT(node_in=node_feats.shape[2], graph_in=graph_feats.shape[1], hidden=256, heads=4, dropout=0.03).to(device)
...
plt.title('UltraEnhancedGAT: Actual vs Predicted VFE')
plt.grid(True)
plt.tight_layout()
plt.show()
